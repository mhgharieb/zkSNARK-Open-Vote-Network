...
template encryptedVoteGen(n){
    ...
    */
    ...
    // hash the concatination of VotingKeysY
    component HashVotingKeysY = HashInputs2(n);
    for (var i=0; i<n; i++){
        HashVotingKeysY.in[i] <== VotingKeysY[i];
    }
    hashVotingKeysY <== HashVotingKeysY.out;
...
component main{public [Idx]} = encryptedVoteGen(__NVOTERS__);