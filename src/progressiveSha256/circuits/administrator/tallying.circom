...
template tallying(n){
    ...
    signal output hash_encryptedVotes;
    ...
    component Hash_encryptedVotes = HashInputs3(n);
    for (var i=0; i<n; i++){
        Hash_encryptedVotes.in[i][0] <== encryptedVotesX[i];
        Hash_encryptedVotes.in[i][1] <== encryptedVotesY[i];
    }
    // for (var i = 0; i< sign2Num_size;i++){
    //     Hash_encryptedVotesY_SignNumX.in[i+n] <== signNumX[i];
    // }
    hash_encryptedVotes <== Hash_encryptedVotes.out;
